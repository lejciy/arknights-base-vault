...
- 其 `base/机制-*.md` 是本库 `docs/0-规则/` 的 RAG 散文清洗重写版：去掉 front matter 与求解向公式、剥离决策经验，来源改为 PRTS 2026-09-03 重抓（本库原文为 2026-06-21 公孙长乐/PRTS 混合源）。**未引入本库**，可作为机制文档逐篇校对的新抓取对照。
...
2. **组合命名**：其 guides 实体化了练卡推荐《组合知识库》的改名裁决（感知信息组/龙舌兰组/格拉斯哥帮组、但书为全基建核心散件），与本库 `docs/2-体系/` 旧命名文档现已实体共存于模块3。
